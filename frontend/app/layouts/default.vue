      ...
      <div class="container mx-auto px-4 flex justify-between items-center h-16">
        ...
        <nav class="flex items-center gap-3">
          <template v-if="!auth.token.value">
            <Button label="登入" as="router-link" to="/auth/login" variant="text" severity="secondary" />
            <Button label="註冊" as="router-link" to="/auth/register" />
          </template>
          <template v-else>
            <Button label="登出" @click="auth.logout" variant="text" severity="secondary" icon="pi pi-sign-out" />
          </template>
        </nav>
      </div>
    </header>
